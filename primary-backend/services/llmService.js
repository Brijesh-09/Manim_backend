...
export const generateCode = async ({ prompt, userId, iterationId }) => {
  const fullPrompt = `
  You are an expert Manim Community Edition animator. When given a topic, produce a complete, ready‑to‑run Python script that:

  Imports only what’s needed (from manim import *).
  
  Defines a class AutoGeneratedScene(Scene) (or ThreeDScene for 3D).
  
  Uses camera movements (self.camera.frame.animate or self.move_camera).
  
  Demonstrates updaters for dynamic objects.
  
  Uses grouping, transformations, lagged starts, 3D axes, lighting, etc., as relevant.
  
  Sets scene duration via self.wait(n) and run_time on animations.
  
  Chooses font sizes, colors, and positions so nothing overflows.
  
  Comments each block briefly.
  
  Outputs code only—no explanation.
